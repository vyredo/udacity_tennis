...
class DDPGAgent:
    def __init__(self, state_size, action_size, random_seed, device, 
        ...
        # Critic Network
        self.critic_local = Critic(state_size , action_size, random_seed, num_agents=num_agents).to(device)
        self.critic_target = Critic(state_size , action_size, random_seed, num_agents=num_agents).to(device)
        self.critic_optimizer = optim.Adam(self.critic_local.parameters(), lr=self.learning_rate * 3, weight_decay=0)
        self.critic_target.load_state_dict(self.critic_local.state_dict())


        self.memory = ReplayBuffer(action_size * num_agents, self.buffer_size, BATCH_SIZE, random_seed)
    ...
    def learn(self, experiences, gamma):
        ...
        actions_next = torch.cat([actions_next_agent1, actions_next_agent2], dim=1)
        ...
        actions_pred = torch.cat([actions_pred_agent1, actions_pred_agent2], dim=1)
